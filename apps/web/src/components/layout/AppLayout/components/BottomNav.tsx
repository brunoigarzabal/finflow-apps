            ...
            {SETTINGS_ITEMS.map(({ to, label, icon }) => (
              <Fragment key={to}>
                <button
                  type="button"
                  className={cn(
                    'flex w-full cursor-pointer items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-accent hover:text-foreground',
                    pathname === to && 'bg-accent font-medium text-foreground'
                  )}
                  onClick={() => {
